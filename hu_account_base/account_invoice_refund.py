...
class account_invoice(models.Model):
    ...
    _journal_invoice_type_dict = {
                'sale': 'out_invoice',  # Customer Invoice
    ...
    def refund(self, date=None, period_id=None, description=None, journal_id=None):
        ...
        for invoice in res:
            #!a
            # error - keyerror 'cash'
            j_type = invoice.journal_id.type
            if j_type in self._journal_invoice_type_dict:
                invoice.write({'type': self._journal_invoice_type_dict[j_type]})
...
class account_invoice_refund(models.TransientModel):
    ...
    def compute_refund(self, cr, uid, ids, mode='refund', context=None):
        ...
        #!a-
        storno_ids = res['domain'][1][2]
        orig_ids = context.get('active_ids')
        inv_rel_obj = self.pool.get('account.invoice.relation')
        inv_rel_obj.create(cr, uid, 
                           {'name':'storno',
                            'source_inv_id':orig_ids[0],
                            'derived_inv_id':storno_ids[0],
                           }, context=context)
        inv_rel_obj.create(cr, uid, 
                           {'name':'str_orig',
                            'source_inv_id':storno_ids[0],
                            'derived_inv_id': orig_ids[0],
                           }, context=context)
        #!-a        
        ...
        id_x = result and result[1] or False
        result = act_obj.read(cr, uid, id_x, context=context)        
